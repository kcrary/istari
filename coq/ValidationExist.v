...
4:{
  ...
  {
  apply (tr_subtype_elim _ (subst1 (ppi1 (var 0)) (subst (under 1 sh1) b))).
    {
    cbn [Nat.add].
    simpsub.
    replace (deq triv triv (subtype (subst (dot (ppi1 (var 0)) sh1) b) (subst (dot (ppi1 (var 0)) sh1) b'))) with (substj (dot (ppi1 (var 0)) id) (deq triv triv (subtype (subst (under 1 sh1) b) (subst (under 1 sh1) b')))) by (simpsub; auto).
...
Lemma prodExistsEq_valid: prodExistsEq_obligation.
  ...
  valid_rewrite. 
  constructor; eauto using deqtype_intro, tr_eqtype_reflexivity.
  Qed.
