...
structure Pickle :> PICKLE =
   struct
      ...
      val w00 = Word8.fromInt 0
      val w01 = Word8.fromInt 1
      val w07 = Word8.fromInt 7
      val w40 = Word8.fromInt 0x40
      val w80 = Word8.fromInt 0x80
      ...
      *)
      ...
      val mask1 : IntInf.int = IntInf.fromInt (0x40 - 1)
      val maskn : IntInf.int = IntInf.fromInt (0x80 - 1)

      fun pIntInfLoop outf n =
         if IntInf.<= (n, maskn) then
      ...
      fun pIntInf outf n =
         let
            val sign = if IntInf.< (n, zero) then signbit else w00
            val n = IntInf.abs n
         in
            if IntInf.<= (n, mask1) then
      ...
      fun uIntInfLoop inf acc =
         let
            val b = inf ()
         in
            if Word8.andb (b, morebitn) = w00 then
      ...
      fun uIntInf inf =
         let
            ...
            val n =
               if Word8.andb (b', morebit1) = w00 then
                  ConvertWord.word8ToIntInf b'
               else
                  IntInf.+
                     (IntInf.<<
                         (foldl
                             (fn (c, n) => IntInf.+ (IntInf.<< (n, 0w7), c))
                             zero
                             (uIntInfLoop inf []),
                          0w6),
                      ConvertWord.word8ToIntInf (Word8.andb (b', Word8.notb morebit1)))
         in
            if Word8.andb (b, signbit) = w00 then
               n
            else
               IntInf.~ n
         end
      ...
      val highest = IntInf.fromInt (Option.valOf Int.maxInt)
      val lowest = IntInf.fromInt (Option.valOf Int.minInt)

      fun uInt inf = 
         let
            val x = uIntInf inf
         in
            if IntInf.< (x, lowest) orelse IntInf.> (x, highest) then
               raise Error
            else
               IntInf.toInt x
         end
      ...
      fun option (PU { pick, unpick, cleanup }) =
         PU { pick =
                 (fn outf => fn admf => (fn NONE => outf w00
                                          | SOME x =>
                                               (
                                               outf w01;
                                               pick outf admf x
      ...
      structure B
         ...
         struct
            ...
            fun new () : working = (ref 0, ref 0, ref (A.array (framesize, w00)), ref nil)

            fun write (size, pos, front, rest) b =
               if !pos >= framesize then
                  let
                     val a = A.array (framesize, w00)
                  in
                     A.update (a, 0, b);
                     pos := 1;
            ...
            val empty = Word8ArraySlice.full (Word8Array.array (0, w00))

            fun front i arr buf =
               if i >= AS.length arr then
                  (case buf of
                      nil => NONE
                    | h :: t => front 0 h t)
               else
