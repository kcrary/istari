...
signature DICT_PLUS =
   sig
      ...
      (* deletes the entry if the merge function returns NONE *)
      val union' : 'a dict -> 'a dict -> (key * 'a * 'a -> 'a option) -> 'a dict

   end


functor ListDict (structure Key : ORDERED)
   :> DICT_PLUS where type key = Key.t
   =
   struct
         ...
         (case (l1, l2) of
           ...
           | ((key1, x) :: rest1, (key2, y) :: rest2) =>
                (case Key.compare (key1, key2) of
                  ...
                  | EQUAL =>
                       (case merge (key1, x, y) of
                           SOME z =>
                              (key1, z) :: union' rest1 rest2 merge

                         | NONE =>
                              union' rest1 rest2 merge)
