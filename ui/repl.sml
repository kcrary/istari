...
functor ReplFun (structure Platform : PLATFORM
   ...
   struct
      ...
      (* Invariant: this contains a native path, not (necessarily) a Unix path.
         We never manipulate it, so it's not worth the trouble of converting it.
      *)
      ...
      fun execute sourcename source p =
         ...
         in
            ...
            (Platform.use (!theTempFilename)  (* native path *)
      ...
      fun ippuse filename runCommands =
         let
            val filename =
               Basis.Path.canonize filename
               handle Basis.Path.Path =>
                  raise (Error.GeneralError ("bad path name " ^ filename))

            val olddir = Basis.FileSystem.getDir ()
         in
            Finally.finally
               (fn () =>
                   let
                      val (newdir, filenameLocal) = Basis.Path.split filename
          
                      val () =
                         Basis.FileSystem.chDir newdir
                         handle exn => handleExn exn
                      ...
                      val ins =
                         Basis.TextIO.openIn filenameLocal
                         handle exn => handleExn exn
                   ...
                   in
                      Finally.finally
                         ...
                         (fn () => Basis.TextIO.closeIn ins)
                   end)
               (fn () => Basis.FileSystem.chDir olddir)
            end
      ...
      *)
      ...
      fun activate main =
         let
            val tempFilename = OS.FileSys.tmpName ()  (* native path *)
      ...
      structure Ctrl =
         struct
            ...
            val pwd = Basis.FileSystem.getDir
            val cd = Basis.FileSystem.chDir
